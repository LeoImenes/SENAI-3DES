...
-- Escreva um select para mostrar o nome do funcionário que mais fez solicitações (em Reais). 
--Então, você deverá mostrar o nome do funcionário e seu respectivo valor total, em Reais. 

SELECT 
    F.nome_func ,
    Concat("R$",MAX(I.valor)) as Valor
    from funcionarios F 
    inner join solicitacao S on F.cod_func = S.cod_func
    inner join itensSolicitacao I on S.Num_Sol = I.Num_Sol;

--Escreva um comando select para mostrar os nomes dos departamentos que requisitaram Parafuso e Difusor.
Select
    D.Nome_Depto,
    P.Nome_Produto
    ...
    where p.nome_produto like "Parafuso" or p.nome_produto like "Difusor" order by p.nome_produto;
    
    
--Escreva um comando select para mostrar os nomes dos produtos vendidos em Fevereiro de 2018?
